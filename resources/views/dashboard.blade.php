            ...
            @foreach($imagens as $imagem)
                <div class="photo-gallery-item">
                    <div class="photo-card h-100 shadow-sm">
                        ...
                        {{-- Use a facade Storage para gerar a URL correta para arquivos no disco 'public' --}}
                        <img src="{{ Storage::url($imagem->path) }}" class="photo-card-img" alt="{{ $imagem->name }}">

                        <div class="photo-card-body">
                            <p class="photo-card-text">Enviada em: {{ $imagem->created_at->format('d/m/Y') }}</p>
                            <p class="photo-card-text"><small class="text-muted">Usuário: {{ $imagem->user->name }}</small></p>
                        </div>
